...
menutrans Split\ &Diff\ with\.\.\.	分割比较(&D)\.\.\.
menutrans Split\ Patched\ &By\.\.\.	分割打补丁(&B)\.\.\.
...
menutrans Toggle\ &Showing\ Matched\ Pairs<Tab>:set\ sm!	开/关显示配对(&S)<Tab>:set\ sm!
...
menutrans Toggle\ Vi\ C&ompatibility<Tab>:set\ cp!	开/关\ Vi\ 兼容(&o)<Tab>:set\ cp!
...
menutrans Toggle\ Tab\ &Expanding<Tab>:set\ et!		开/关制表扩展(&E)<Tab>:set\ et!
menutrans Toggle\ &Auto\ Indenting<Tab>:set\ ai!	开/关自动缩进(&A)<Tab>:set\ ai!
menutrans Toggle\ &C-Style\ Indenting<Tab>:set\ cin!	开/关\ C\ 缩进(&C)<Tab>:set\ cin!
...
menutrans &Jump\ to\ this\ tag<Tab>g^]	跳转到这个标签(&J)<Tab>g^]
menutrans Jump\ &back<Tab>^T		跳转返回(&b)<Tab>^T
menutrans Build\ &Tags\ File		建立标签文件(&T)
...
menutrans Fold\ Col&umn\ Width		折叠栏宽度(&u)

" Tools.Diff Menu
menutrans &Diff				比较(&D)
menutrans &Update			更新(&U)
menutrans &Get\ Block			获取块(&G)
...
menutrans Conve&rt\ back<Tab>:%!xxd\ -r	转换回来(&r)<Tab>:%!xxd\ -r
